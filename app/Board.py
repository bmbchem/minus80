...
class Board:
    ...
    def alert(self):
        ...
        event.alert(self.__ip, board_logger, self.get_hostname())
    ...
    def check_alert(self, event):
        ...
        ten minutes for an hour or until recovery is detected."""
        
        # Get board logger
        board_logger = self.get_board_logger()

        # Loop for up to one hour and continue to alert every ten minutes.
        start_time = datetime.now()
        end_time = start_time + timedelta(hours=1)

        alarm_counter = 0
        while alarm_counter < 6 and datetime.now() < end_time:
            # Sleep for 10 minutes before the next reminder.
            sleep(600)
